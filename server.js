...
app.post('/vote/:email/:user', async (req, res) => { // new endpoint to increase vote
  try {
    ...
    if (voter === candidate) return res.status(500).send(`You cannot vote for yourself !`)
    ...
    console.log(`The previously voted for candidates ${voted_for} for candidate ${candidate}`)

    console.log('foo')
    // if email voted for === users.email ==> Error

    
    if( candidates_voted_for > 2) return res.status(500).send(`Limit of vote reached !`)

    let cond_one_vote_per_candidate = (uniqueVotes < candidates_voted_for && uniqueVotes == 1)
    
    console.log(`uniqueVotes = ${uniqueVotes} & voted_for.length = ${candidates_voted_for} cond = ${cond_one_vote_per_candidate}`)


    if( cond_one_vote_per_candidate ) return res.status(500).send(`Cannot vote twice for the same candidate !`)

    // find user by email and then increment the votes attribute by 1
    await User.findOneAndUpdate({ email: candidate }, { $inc: { votes: 1 } });

    await User.findOneAndUpdate({ email: voter }, { $push: { voted_for: candidate } })
...
app.post('/unvote/:email/:user', async (req, res) => { // new endpoint to increase vote
  try {
    ...
    if (voter_voted_for_candidate) {
        ...
        await User.findOneAndUpdate({ email: voter }, { $pull: { voted_for: candidate } })
...
app.post('/becandidate/:email', async (req, res) => {
    ...
    if (user.candidate) return res.status(400).send('You are already a candidate');
  
    try {
    await User.findOneAndUpdate({ email: req.params.email }, { candidate: true });
...
app.post('/withdraw/:email', async (req, res) => {
    ...
    const user = await User.findOne({ email: req.params.email });
    if (!user.candidate) return res.status(400).send('You are already withdrawed');
  
    try {
    await User.findOneAndUpdate({ email: req.params.email }, { candidate: false });
    res.send('Candidate status updated');
  } catch (error) {
    res.status(500).send(error);
  }
